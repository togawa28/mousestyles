...
def test_compute_accelerations_input():
    ...
    with pytest.raises(TypeError) as excinfo:
        path_diversity.compute_accelerations(0, 1)
    ...
    with pytest.raises(ValueError) as excinfo:
        path_diversity.compute_accelerations([0, 1], [1, 2])
    ...
    with pytest.raises(ValueError) as excinfo:
        path_diversity.compute_accelerations([1, 3], [4, 4, 4])
    assert excinfo.value.args[0] == test


def test_compute_accelerations():
    assert path_diversity.compute_accelerations([1, 2, 0],
                                                [3, 4, 5, 6]) == [0.5, -1.0]
...
def test_angle_between_input():
    # Check if function raises the correct type of errors.
    with pytest.raises(TypeError) as excinfo:
        path_diversity.angle_between([0], 1)
    assert excinfo.value.args[0] == 'v1 and v2 must be lists'

    with pytest.raises(ValueError) as excinfo:
        path_diversity.angle_between([0], [1, 2])
    assert excinfo.value.args[0] == 'both vectors must have same lengths'

    with pytest.raises(ValueError) as excinfo:
        path_diversity.angle_between([0, 0], [1, 0])
    assert excinfo.value.args[
        0] == 'both vectors must have norms greater than 0'


def test_angle_between():
    assert path_diversity.angle_between([0, 1], [0, 1]) == 0.0
    assert path_diversity.angle_between([1, 0], [0, 1]) == np.pi / 2
    assert path_diversity.angle_between([0, 1], [0, -1]) == np.pi
